...
def test_urgent_item_processed_first():
    """긴급 안건이 리스트 순서와 무관하게 먼저 배정 + 짐(load) 반영 분산.

    r0@(0,0), r1@(60,0); cA@(25,0) u=1, cB@(35,0) u=9 — 리스트에는 cA가 먼저.
    긴급 우선: cB부터 — r1(75) < r0(105) → r1 낙찰, r1 load=75+300.
    cA: r0(75) < r1(load 375+30=405) → r0. 짐 없는 로봇이 분산 수주 (구사양은 r1 독식).
    """
...
def test_chained_position_used_for_next_bid():
    """연쇄 위치는 유지되나 짐(load)이 지배 — 두 번째 안건은 자유 로봇에게.

    r0@(0,0), r1@(100,0); c1@(60,0) u=5 → r1 낙찰(120 < 180), load=120+300.
    c2@(35,0) u=3 — r1은 연쇄 위치(60,0)에서 75지만 load 420 합산 495 > r0 105 → r0.
    (작업 시간이 생긴 세계에선 한 로봇 독식이 파트너 대기를 만들므로 의도된 변화.)
    """
...
def test_multi_winner_all_chained():
    ...
    c2@(5,30) n̂=1 — r0·r1은 짐(315·321)에 눌리고, 멀지만 자유로운 r2(299)가 수주.
    """
