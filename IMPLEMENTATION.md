...
| 27 | 같은 씨앗 재실행이 다른 결과 (v2 상차 회귀로 위장) | 맹점 추첨의 hash()가 문자열 밴드에 프로세스별 무작위 시드 — 재현성·짝 비교 파괴 (v1 본 실험에도 잡음 혼입) | 정수 연산 해시로 교체 + 프로세스 간 일치 검증 |
| 24 | 만남 70% 무산 (성사 5/시도 21) | 로봇이 약속 시각에야 출발 — 이동 최대 10분 > 유예 4분 | 지각 예상 로봇만 조기 직행(리드타임) + 무산 시 절반 간격 재시도 |
| 25 | 허탕 많은 팀이 완료율 과대 | **유령 도착자**: 인원부족 복귀한 로봇이 도착 명단에 잔존 → 원격 작업 참여 | bounce 시 명단 제거 |
| 26 | 배정 도착 로봇이 엉뚱한 판단 | 기억 번호(cid)가 로봇별 일련번호인데 회의 주재자 번호로 조회 | 위치 기준 후보 식별 (번호+위치 일치 우선) |
